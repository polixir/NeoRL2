...
patient_action = namedtuple("patient_action", ['CHO', 'insulin'])
patient_observation = namedtuple("patient_observation", ['Gsub'])


dir, _ = os.path.split(os.path.abspath(__file__))

PATIENT_PARA_FILE = os.path.join(dir, 'vpatient_params.csv')
...
class T1DPatient(Patient):
    ...
    def observation(self):
        ...
        observation = patient_observation(Gsub=Gsub)
        return observation
    ...
    def reset(self):
        ...
        self._last_action = patient_action(CHO=0, insulin=0)
        self.is_eating = False
        self.planned_meal = 0
...
if __name__ == '__main__':
    ...
    while p.t < 1000:
        ...
        # if p.t == 150:
        #     ins = 80.0 / 12.0 + basal
        act = patient_action(insulin=ins, CHO=carb)
        t.append(p.t)
        CHO.append(act.CHO)
