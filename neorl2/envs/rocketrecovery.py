...
from .reward.rocketrecovery_reward import get_reward
from .terminated.rocketrecovery_terminated import get_terminated
...
class RocketRecoveryEnv(Env, EzPickle):
    """
    ## Description
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def step(self, action):
        ...
        if abs(state[1]) <= 0.05:
            terminated = True
            if abs(state[4]) < 0.3 and abs(state[2]) < 0.5 and abs(state[3]) < 2:
                reward = +100
            else:
                reward = -100
